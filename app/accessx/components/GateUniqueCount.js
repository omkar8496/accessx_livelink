  ...
  return (
    <div className="rounded-3xl border border-[color:var(--border-light)] bg-[color:var(--bg-secondary)] p-6 shadow-[var(--shadow-lg)] hover:shadow-[var(--shadow-xl)] transition-all">
      <div className="mb-4 flex items-center justify-between">
        ...
        <div className="grid grid-cols-2 text-xs font-bold uppercase tracking-wide min-w-[140px] text-right">
      ...
      {loading && <p className="text-sm text-[color:var(--text-secondary)]">Loading gates…</p>}

      {!loading && rows.length === 0 && !error && (
        <p className="text-sm text-[color:var(--text-secondary)]">No gate data.</p>
      )}
